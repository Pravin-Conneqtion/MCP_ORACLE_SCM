...
@mcp.tool()
async def get_all_locations() -> Dict[str, Any]:
    """Fetch all Locations from Fusion (default DEV1 environment).

    Saves results into both JSON and CSV files under ./output directory.
    """
    try:
        Logger.log("Starting get_all_locations", level="INFO")
        location_service = get_location_service()
        result = await location_service.fetch_all_locations()
        Logger.log("Completed get_all_locations", level="INFO", result=result)
        return result
    except Exception as e:
        Logger.log("Error in get_all_locations", level="ERROR", error=str(e))
        return {"error": str(e), "error_type": type(e).__name__}
    
@mcp.tool()
async def migrate_locations(target_env: str = "DEV1") -> Dict[str, Any]:
    """Migrate locations from source (DEV1) to target (e.g. TEST or PROD)."""
    try:
        Logger.log("Starting migrate_locations", level="INFO", target_env=target_env)
        location_service = get_location_service()
        result = await location_service.migrate_locations_to_target_instance(target_env)
        Logger.log("Completed migrate_locations", level="INFO", result=result)
        return result
    except Exception as e:
        Logger.log("Error in migrate_locations", level="ERROR", error=str(e))
        return {"error": str(e), "error_type": type(e).__name__}
